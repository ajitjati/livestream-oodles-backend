 ...
 *                 accessed via ws://localhost:8080/jWebrtc/ws Where "localhost"
 *                 is the address of the host, "jWebrtc" is the name of the
 *                 package and "ws" is the address to access this class from the
 *                 server
 */
@ServerEndpoint("/ws")
public class WebSocketServer {
		...
		case "register":
			try {
				...
				if (registered) {
					...
					sendRegisteredUsers(userSession, jsonMessage);
	...
	private void queryOnlineStatus(Session session, JsonObject jsonMessage) throws IOException {
		...
		registry.addVisitor(user, myUserSession);
		...
		if (userSession == null) {
			...
			sendVisitorListToSiteOwner(userSession);
		}
	...
	public void publishOnlineStatus(String user, String status) throws IOException {
		...
		for (UserSession userSession : registry.getUserSessions()) {
			responseJSON.addProperty("myUsername", userSession.getName()); // include
																			// my
																			// online
																			// sessinID
	...
	private void appConfig(Session session, JsonObject jsonMessage) throws IOException {
		...
		String stun = "{" + "\"username\":\"\"," + "\"password\":\"\"," + "\"urls\":[" + "\"" + stunUrl
				+ "?transport=udp\",\"" + stunUrl + "?transport=tcp\"";
		...
		String turn = "{" + "\"username\":\"" + turnUsername + "\"," + "\"password\":\"" + turnPassword + "\","
				+ "\"urls\":["
		...
		String responseJSON = "{" + "\"params\" : {" + "\"pc_config\": {\"iceServers\": " + iceConfig + "}" + // ,
																												// \"iceTransportPolicy\":
																												// \"relay\"
				"}," + "\"result\": \"SUCCESS\"" + "}";
	 ...
	 * @throws IOException
	 */
	private void sendRegisteredUsers(UserSession registeredUser, JsonObject jsonMessage) throws IOException {
		if (jsonMessage != null && jsonMessage.has("APP_ID") && registry.isSiteOwner(registeredUser.getName())) {
			String appId = jsonMessage.getAsJsonPrimitive("APP_ID").getAsString();
			log.debug("APP_ID = {}", appId);

			sendVisitorListToSiteOwner(registeredUser);
			return;
		}
		...
		for (UserSession userSession : registry.getUserSessions()) {
			if (userSession.getSession().isOpen()) {
				...
				log.warn("removing session id from registry because it's not open {}", userSession.getSession());
				registry.removeBySession(userSession.getSession());
			}
		}
	}

	private void sendVisitorListToSiteOwner(UserSession owner) throws IOException {
		List<String> userList = new ArrayList<>();
		for (UserSession session : registry.getVisitors(owner.getName())) {
			userList.add(session.getName());
		}
		...
		log.info("Updating user list for site owner {}: {}", owner.getName(), responseJSON);

		if (owner.getSession().isOpen()) {
			owner.sendMessage(responseJSON);
		} else {
			log.warn("removing session id from registry because it's not open {}", owner.getSession());
			registry.removeBySession(owner.getSession());
		}
	...
	public void stop(Session session) throws IOException {
		...
		if (stopperUser != null) {
			...
			if (stoppedUserFrom != null && stoppedUserFrom.getSession() != null
				...
				log.debug("die id des stoppenden ist NICHT! die des anrufenden"); // wenn
																					// der
																					// angerufene
																					// auflegt
				...
				// wenn der anrufer auflegt. (wird anschließend, die pipeline
				// des anrufenden gesucht und exisitert nicht mehr)
		...
		// }
		// else{ //piplines not yet have been created - but a user tried to call
		// another and the other hangs up instead of answers the call
	...
	public void stopScreen(Session session) throws IOException {
		...
		if (stoppedUserFrom != null && stoppedUserFrom.getSession() != null
			...
			log.debug("die id des stoppenden IST! die des anrufenden");
			// wenn der anrufer auflegt. (wird anschließend, die pipeline des
			// anrufenden gesucht und exisitert nicht mehr)
			stopUser = stoppedUserTo;
		}
